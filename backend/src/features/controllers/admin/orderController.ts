// src/features/controllers/admin/orderController.ts
    ...
    try {
        const updatedOrder = await prisma.$transaction(async (tx) => {
            ...
            if (oldStatus === 'pendiente' && Array.isArray(newItems)) {
                ...
                const newItemsMap = new Map(newItems.map((item: any) => [item.producto_id, item.quantity]));
                ...
                for (const productoId of allProductIds) {
                    ...
                    if (delta > 0) {
                        const product = await tx.producto.findUnique({ where: { id: productoId } });
                        if (product && product.tracks_stock) {
                ...
                await tx.detallePedido.deleteMany({ where: { pedido_id: Number(id) } });
                await tx.detallePedido.createMany({
            ...
            const updatedOrderData = await tx.pedido.update({
                ...
                data: {
                    ...
                    total_pago: total_pago ?? order.total_pago,
                },
            });
